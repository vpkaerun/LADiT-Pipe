...
from ladit_pipe.utils.ffmpeg_wrapper import convert_to_wav
from ladit_pipe.utils.file_handler import find_audio_files
from ladit_pipe.presets.walking import create_intelligent_chunks
from ladit_pipe.core.diarization import diarize_chunk
from ladit_pipe.core.transcription import transcribe_chunk
from ladit_pipe.core.export import merge_results  # 追加

# 設定定数
# SUPPORTED_AUDIO_FORMATS = {'.wav', '.mp3', '.m4a', '.flac', '.ogg'} # 移動済み
# SUPPORTED_VIDEO_FORMATS = {'.mkv', '.mp4', '.mov', '.avi', '.webm'} # 移動済み
TARGET_SAMPLE_RATE = 16000
# CHUNK_LENGTH_MS = 30000  # 30秒 # 移動済み
# MAX_CHUNK_LENGTH_MS = 60000  # 60秒 # 移動済み
# MIN_SILENCE_LEN = 500  # 0.5秒 # 移動済み
# SILENCE_THRESH = -40  # dB # 移動済み
...
class AudioProcessor:
    ...
    def initialize_models(
        ...
        self.logger.info(
            f"話者分離パラメータ設定: min_speakers={min_speakers}, max_speakers={max_speakers}"
        )
    ...
    # create_intelligent_chunks メソッドは ladit_pipe.presets.walking に移動済み
    ...
    # _split_by_time メソッドは ladit_pipe.presets.walking に移動済み
    ...
    # diarize_chunk メソッドは ladit_pipe.core.diarization に移動済み
    ...
    # _improve_speaker_consistency メソッドは ladit_pipe.core.diarization に移動済み
    ...
    # transcribe_chunk メソッドは ladit_pipe.core.transcription に移動済み
    ...
    # _post_process_transcription メソッドは ladit_pipe.core.transcription に移動済み
    ...
    # _remove_repetitions メソッドは ladit_pipe.core.transcription に移動済み
    ...
    # _is_meaningless_segment メソッドは ladit_pipe.core.transcription に移動済み
    ...
    def process_file(
        self, input_file: Path, output_dir: Path, state: ProcessingState
    ) -> bool:
        """単一ファイルを処理"""
        ...
        try:
            ...
            # 1. WAV変換
            wav_file = convert_to_wav(input_file, self.temp_dir)

            # 2. チャンク分割
            chunks_info = create_intelligent_chunks(wav_file, self.temp_dir)
            ...
            with tqdm(chunks_info, desc=f"Processing {input_file.name}") as pbar:
                for i, (chunk_file, chunk_start, chunk_end) in enumerate(pbar):
                    ...
                    # 文字起こし
                    transcription = transcribe_chunk(chunk_file, self.whisper_model)

                    chunk_results.append(
                        (transcription, diarization, chunk_start, chunk_end)
                    )
            ...
            output_files = merge_results(chunk_results, output_base)  # 修正
...
# find_audio_files 関数は ladit_pipe.utils.file_handler に移動済み
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", type=Path, default="output", help="出力ディレクトリ"
    )
    ...
    try:
        ...
        if args.sensitive and hasattr(processor, "diarization_pipeline"):
            if hasattr(processor.diarization_pipeline, "_segmentation"):
                ...
                processor.diarization_pipeline._segmentation.offset = (
                    0.2  # さらに敏感に
                )
